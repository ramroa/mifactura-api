package mx.mifactura.api.service;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.http.HttpEntity;
import org.springframework.http.HttpHeaders;
import org.springframework.http.ResponseEntity;
import org.springframework.stereotype.Service;
import org.springframework.web.client.RestTemplate;

import mx.mifactura.api.providers.docdigitales.descargar.request.DescargarRequest;
import mx.mifactura.api.providers.docdigitales.descargar.response.DescargarResponse;
import mx.mifactura.api.providers.docdigitales.model.ErrorResponse;
import mx.mifactura.api.providers.docdigitales.model.GenerarFacturaRequest;
import mx.mifactura.api.providers.docdigitales.model.Meta;

@Service
public class DocDigitalesFacturaService implements IFacturaService {
	
	@Autowired
	private RestTemplate restTemplate;
	
	@Value("${docdigitales.sandbox.key}")
	private String apiKey;

	public ErrorResponse generarFactura(GenerarFacturaRequest request) {
		HttpHeaders headers = new HttpHeaders();
		headers.set("Authorization", "Token token=" + apiKey);
		
		Meta meta = new Meta();
		meta.setAmbiente("S");
		meta.setEmpresa_api_key("");
		meta.setEmpresa_uid("4abb55fe1c");
		meta.setObjeto("factura");
		request.setMeta(meta);
		
		
		HttpEntity<GenerarFacturaRequest> entity = new HttpEntity<GenerarFacturaRequest>(request, headers);
		
		ResponseEntity<ErrorResponse> response = restTemplate.postForEntity("https://api.docdigitales.com/v1/facturas/generar",entity, ErrorResponse.class);
		
		if(response.getStatusCode().is2xxSuccessful()) {
			ErrorResponse errorResponse = response.getBody();
			return errorResponse;
		} else {
			return null;
		}
	}

	@Override
	public DescargarResponse descargarFactura(DescargarRequest request) {
		HttpHeaders headers = new HttpHeaders();
		headers.set("Authorization", "Token token=" + apiKey);
		HttpEntity<DescargarRequest> entity = new HttpEntity<DescargarRequest>(request, headers);

		ResponseEntity<DescargarResponse> response = restTemplate.postForEntity("https://api.docdigitales.com/v1/descargar",entity, DescargarResponse.class);

		if(response.getStatusCode().is2xxSuccessful()) {
			return response.getBody();
		} else {
			return null;
		}
	}

}
